...
void TextInit(const char* Font, int SCR_WIDTH, int SCR_HEIGHT, int TextWidth, int TextHeight, Shader &TextShader)
{
    ...
    if (FT_New_Face(ft, Font, 0, &face))
    {
        ...
        FT_Set_Pixel_Sizes(face, textwidth, textheight);
        ...
        for (unsigned char c = 0; c < 128; c++)
        {
            ...
            {
                std::cout << "ERROR::FREETYTPE: Failed to load Glyph2" << std::endl;
                continue;
            }
            // generate texture
...
void RenderText(std::string text, float x, float y, float scale, glm::vec3 color, Shader& TextShader, VAO vao)
{
    ...
    TextShader.Activate();
    vao.Bind();
    VBO vbo(NULL, sizeof(float) * 6 * 4, GL_DYNAMIC_DRAW);
    vao.LinkAttribute(vbo, 0, 4, GL_FLOAT, 4 * sizeof(float), 0);
